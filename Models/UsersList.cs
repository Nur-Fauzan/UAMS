...
public partial class UAMS_20250216_1835 {
    ...
    public class UsersListBase : Users
    {
        ...
        protected bool RestoreFilterList() {
            ...
            // Field PasswordHash
            if (filter?.TryGetValue("x_PasswordHash", out sv) ?? false) {
                PasswordHash.AdvancedSearch.SearchValue = sv;
                PasswordHash.AdvancedSearch.SearchOperator = filter["z_PasswordHash"];
                PasswordHash.AdvancedSearch.SearchCondition = filter["v_PasswordHash"];
                PasswordHash.AdvancedSearch.SearchValue2 = filter["y_PasswordHash"];
                PasswordHash.AdvancedSearch.SearchOperator2 = filter["w_PasswordHash"];
                PasswordHash.AdvancedSearch.Save();
            }
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fUserslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fUserslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fUserslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
