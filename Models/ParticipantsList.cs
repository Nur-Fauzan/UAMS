...
public partial class project1 {
    ...
    /// participantsList
    /// </summary>
    public static ParticipantsList participantsList
    {
        get => HttpData.Get<ParticipantsList>("participantsList")!;
        set => HttpData["participantsList"] = value;
    }

    /// <summary>
    /// Page class for Participants
    /// </summary>
    public class ParticipantsList : ParticipantsListBase
    {
        // Constructor
        public ParticipantsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ParticipantsList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class ParticipantsListBase : Participants
    {
        // Page ID
        ...
        public string FormName = "fParticipantslist";
        ...
        // Constructor
        public ParticipantsListBase()
        {
            TableName = "Participants";

            // CSS class name as context
            TableVar = "Participants";
            ...
            MultiDeleteUrl = "ParticipantsDelete";
            MultiUpdateUrl = "ParticipantsUpdate";
        ...
        // Page name
        public string PageName => "ParticipantsList";
        ...
        // Constructor
        public ParticipantsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            // Field AppointmentId
            if (filter?.TryGetValue("x_AppointmentId", out sv) ?? false) {
                AppointmentId.AdvancedSearch.SearchValue = sv;
                AppointmentId.AdvancedSearch.SearchOperator = filter["z_AppointmentId"];
                AppointmentId.AdvancedSearch.SearchCondition = filter["v_AppointmentId"];
                AppointmentId.AdvancedSearch.SearchValue2 = filter["y_AppointmentId"];
                AppointmentId.AdvancedSearch.SearchOperator2 = filter["w_AppointmentId"];
                AppointmentId.AdvancedSearch.Save();
            }
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fParticipantslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fParticipantslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fParticipantssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fParticipantssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fParticipantslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_Participants");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(participantsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(participantsList.RowCount) + "_Participants");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(participantsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && participantsList.RowType == RowType.Add || IsEdit && participantsList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fParticipantssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
