...
public partial class UAMS_20250216_1835 {
    ...
    {
        get => HttpData.Resolve<Timezones>("Timezones");
        set => HttpData["Timezones"] = value;
    }

    /// <summary>
    /// Table class for Timezones
    /// </summary>
    public class Timezones : DbTable
    {
        ...
        public readonly DbField<SqlDbType> TimezoneID;

        public readonly DbField<SqlDbType> TimezoneName;

        public readonly DbField<SqlDbType> UtcOffset;
        ...
        public Timezones()
        {
            ...
            Name = "Timezones";
            Type = "TABLE";
            UpdateTable = "dbo.Timezones"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            // TimezoneID
            TimezoneID = new(this, "x_TimezoneID", 3, SqlDbType.Int) {
                Name = "TimezoneID",
                Expression = "[TimezoneID]",
                BasicSearchExpression = "CAST([TimezoneID] AS NVARCHAR)",
                DateTimeFormat = -1,
                VirtualExpression = "[TimezoneID]",
                IsVirtual = false,
                ...
                CustomMessage = Language.FieldPhrase("Timezones", "TimezoneID", "CustomMsg"),
                IsUpload = false
            };
            TimezoneID.Raw = true;
            Fields.Add("TimezoneID", TimezoneID);

            // TimezoneName
            TimezoneName = new(this, "x_TimezoneName", 202, SqlDbType.NVarChar) {
                Name = "TimezoneName",
                Expression = "[TimezoneName]",
                UseBasicSearch = true,
                BasicSearchExpression = "[TimezoneName]",
                DateTimeFormat = -1,
                VirtualExpression = "[TimezoneName]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("Timezones", "TimezoneName", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("TimezoneName", TimezoneName);

            // UtcOffset
            UtcOffset = new(this, "x_UtcOffset", 202, SqlDbType.NVarChar) {
                Name = "UtcOffset",
                Expression = "[UtcOffset]",
                UseBasicSearch = true,
                BasicSearchExpression = "[UtcOffset]",
                DateTimeFormat = -1,
                VirtualExpression = "[UtcOffset]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                Required = true, // Required field
                SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"],
                CustomMessage = Language.FieldPhrase("Timezones", "UtcOffset", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("UtcOffset", UtcOffset);
        ...
        public async Task<int> InsertAsync(object data)
        {
            ...
            if (data is IDictionary<string, object> d)
                ...
                throw new ArgumentException("Data must be of anonymous or Dictionary<string, object> or Entities.Timezone type", "data");
            row = row.Where(kvp => {
            ...
            try {
                ...
                if (row.ContainsKey("TimezoneID"))
                    row["TimezoneID"] = lastInsertedId;
                else
                    row.Add("TimezoneID", lastInsertedId);
                TimezoneID.SetDbValue(lastInsertedId);
                result = 1;
            } catch (Exception e) {
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            if (row == null)
                return;
            try {
                TimezoneID.DbValue = row["TimezoneID"]; // Set DB value only
                TimezoneName.DbValue = row["TimezoneName"]; // Set DB value only
                UtcOffset.DbValue = row["UtcOffset"]; // Set DB value only
            } catch {}
        }
        ...
        private string _sqlKeyFilter => "[TimezoneID] = @TimezoneID@";
        ...
        public string GetModalCaption(string pageName)
        {
            return pageName switch {
                "TimezonesView" => Language.Phrase("View"),
                "TimezonesEdit" => Language.Phrase("Edit"),
                "TimezonesAdd" => Language.Phrase("Add"),
                _ => String.Empty
            };
        }
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "TimezonesView",
                Config.ApiAddAction => "TimezonesAdd",
                Config.ApiEditAction => "TimezonesEdit",
                Config.ApiDeleteAction => "TimezonesDelete",
                Config.ApiListAction => "TimezonesList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "TimezonesList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("TimezonesView", parm);
            else
                url = KeyUrl("TimezonesView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "TimezonesAdd";
        ...
        {
            string url = "";
            url = KeyUrl("TimezonesEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("TimezonesList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("TimezonesAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("TimezonesList", "action=copy"))); // DN
        ...
        public string GetDeleteUrl(string parm = "")
        {
            return UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
                ...
                : AppPath(KeyUrl("TimezonesDelete", parm)); // DN
        }
        ...
        {
            string json = "";
            json += "\"TimezoneID\":" + ConvertToJson(TimezoneID.CurrentValue, "number", true);
            json = "{" + json + "}";
            if (htmlEncode)
        ...
        {
            if (!IsNull(TimezoneID.CurrentValue)) {
                url += "/" + TimezoneID.CurrentValue;
            } else {
        ...
        public List<string> GetRecordKeys()
        {
            ...
            if (Post("key_m[]", out sv) || Get("key_m[]", out sv)) { // DN
                ...
                if (RouteValues.TryGetValue("TimezoneID", out object? v0)) { // TimezoneID // DN
                    key = UrlDecode(v0); // DN
                } else if (IsApi() && !Empty(keyValues)) {
                    key = keyValues[0];
                } else {
                    key = Param("TimezoneID");
                }
                keysList.Add(key);
            }
            // Check keys
        ...
        public string GetFilterFromRecordKeys(bool setCurrent = true)
        {
            ...
            foreach (var keys in recordKeys) {
                if (!Empty(keyFilter))
                    keyFilter += " OR ";
                if (setCurrent)
                    TimezoneID.CurrentValue = keys;
                else
                    TimezoneID.OldValue = keys;
        ...
        public async Task RenderEditRow()
        {
            ...
            TimezoneName.EditValue = TimezoneName.CurrentValue;
            ...
            UtcOffset.EditValue = UtcOffset.CurrentValue;
        ...
        public async Task ExportDocument(dynamic doc, DbDataReader dataReader, int startRec, int stopRec, string exportType = "")
        {
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    doc.BeginExportRow();
                    if (exportType == "view") {
                        doc.ExportCaption(TimezoneID);
                        doc.ExportCaption(TimezoneName);
                        doc.ExportCaption(UtcOffset);
                    } else {
                        doc.ExportCaption(TimezoneID);
                        doc.ExportCaption(TimezoneName);
                        doc.ExportCaption(UtcOffset);
            ...
            do { // DN
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            await doc.ExportField(TimezoneID);
                            await doc.ExportField(TimezoneName);
                            await doc.ExportField(UtcOffset);
                        } else {
                            await doc.ExportField(TimezoneID);
                            await doc.ExportField(TimezoneName);
                            await doc.ExportField(UtcOffset);
