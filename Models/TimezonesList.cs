...
public partial class UAMS_20250216_1835 {
    ...
    /// timezonesList
    /// </summary>
    public static TimezonesList timezonesList
    {
        get => HttpData.Get<TimezonesList>("timezonesList")!;
        set => HttpData["timezonesList"] = value;
    }

    /// <summary>
    /// Page class for Timezones
    /// </summary>
    public class TimezonesList : TimezonesListBase
    {
        // Constructor
        public TimezonesList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TimezonesList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TimezonesListBase : Timezones
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "timezonesList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fTimezoneslist";
        ...
        public TimezonesListBase()
        {
            ...
            // Table object (timezones)
            if (timezones == null || timezones is Timezones)
                timezones = this;
            ...
            MultiDeleteUrl = "TimezonesDelete";
            MultiUpdateUrl = "TimezonesUpdate";
        ...
        // Page name
        public string PageName => "TimezonesList";
        ...
        {
            TimezoneID.SetVisibility();
            TimezoneName.SetVisibility();
            UtcOffset.SetVisibility();
        }

        // Constructor
        public TimezonesListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "TimezonesView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("TimezoneID") ? dict["TimezoneID"] : TimezoneID.CurrentValue));
            return key;
        }
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(TimezoneID.AdvancedSearch.ToJson())); // Field TimezoneID
            filters.Merge(JObject.Parse(TimezoneName.AdvancedSearch.ToJson())); // Field TimezoneName
            filters.Merge(JObject.Parse(UtcOffset.AdvancedSearch.ToJson())); // Field UtcOffset
        ...
        protected bool RestoreFilterList() {
            ...
            // Field TimezoneID
            if (filter?.TryGetValue("x_TimezoneID", out sv) ?? false) {
                TimezoneID.AdvancedSearch.SearchValue = sv;
                TimezoneID.AdvancedSearch.SearchOperator = filter["z_TimezoneID"];
                TimezoneID.AdvancedSearch.SearchCondition = filter["v_TimezoneID"];
                TimezoneID.AdvancedSearch.SearchValue2 = filter["y_TimezoneID"];
                TimezoneID.AdvancedSearch.SearchOperator2 = filter["w_TimezoneID"];
                TimezoneID.AdvancedSearch.Save();
            }

            // Field TimezoneName
            if (filter?.TryGetValue("x_TimezoneName", out sv) ?? false) {
                TimezoneName.AdvancedSearch.SearchValue = sv;
                TimezoneName.AdvancedSearch.SearchOperator = filter["z_TimezoneName"];
                TimezoneName.AdvancedSearch.SearchCondition = filter["v_TimezoneName"];
                TimezoneName.AdvancedSearch.SearchValue2 = filter["y_TimezoneName"];
                TimezoneName.AdvancedSearch.SearchOperator2 = filter["w_TimezoneName"];
                TimezoneName.AdvancedSearch.Save();
            }

            // Field UtcOffset
            if (filter?.TryGetValue("x_UtcOffset", out sv) ?? false) {
                UtcOffset.AdvancedSearch.SearchValue = sv;
                UtcOffset.AdvancedSearch.SearchOperator = filter["z_UtcOffset"];
                UtcOffset.AdvancedSearch.SearchCondition = filter["v_UtcOffset"];
                UtcOffset.AdvancedSearch.SearchValue2 = filter["y_UtcOffset"];
                UtcOffset.AdvancedSearch.SearchOperator2 = filter["w_UtcOffset"];
                UtcOffset.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(TimezoneName);
            searchFlds.Add(UtcOffset);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(TimezoneID); // TimezoneID
                UpdateSort(TimezoneName); // TimezoneName
                UpdateSort(UtcOffset); // UtcOffset
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""Timezones"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""Timezones"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""Timezones"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fTimezoneslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTimezoneslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(TimezoneID.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""Timezones"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("TimezoneID")); // DN
                CreateColumnOption(option.Add("TimezoneName")); // DN
                CreateColumnOption(option.Add("UtcOffset")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fTimezonessrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fTimezonessrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fTimezoneslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(timezonesList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(timezonesList.RowCount) + "_Timezones");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(timezonesList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && timezonesList.RowType == RowType.Add || IsEdit && timezonesList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            RowSelected(row);
            TimezoneID.SetDbValue(row["TimezoneID"]);
            TimezoneName.SetDbValue(row["TimezoneName"]);
            UtcOffset.SetDbValue(row["UtcOffset"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("TimezoneID", TimezoneID.DefaultValue ?? DbNullValue); // DN
            row.Add("TimezoneName", TimezoneName.DefaultValue ?? DbNullValue); // DN
            row.Add("UtcOffset", UtcOffset.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // TimezoneID
                TimezoneID.ViewValue = TimezoneID.CurrentValue;
                TimezoneID.ViewCustomAttributes = "";

                // TimezoneName
                TimezoneName.ViewValue = ConvertToString(TimezoneName.CurrentValue); // DN
                TimezoneName.ViewCustomAttributes = "";

                // UtcOffset
                UtcOffset.ViewValue = ConvertToString(UtcOffset.CurrentValue); // DN
                UtcOffset.ViewCustomAttributes = "";

                // TimezoneID
                TimezoneID.HrefValue = "";
                TimezoneID.TooltipValue = "";

                // TimezoneName
                TimezoneName.HrefValue = "";
                TimezoneName.TooltipValue = "";

                // UtcOffset
                UtcOffset.HrefValue = "";
                UtcOffset.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fTimezonessrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
