...
public partial class UAMS_20250216_1835 {
    ...
    {
        get => HttpData.Get<AppointmentsList>("appointmentsList")!;
        set => HttpData["appointmentsList"] = value;
    }

    /// <summary>
    /// Page class for Appointments
    /// </summary>
    public class AppointmentsList : AppointmentsListBase
    {
        // Constructor
        public AppointmentsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public AppointmentsList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class AppointmentsListBase : Appointments
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "appointmentsList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fAppointmentslist";
        ...
        // Constructor
        public AppointmentsListBase()
        {
            TableName = "Appointments";

            // CSS class name as context
            TableVar = "Appointments";
            ...
            MultiDeleteUrl = "AppointmentsDelete";
            MultiUpdateUrl = "AppointmentsUpdate";
        ...
        // Page name
        public string PageName => "AppointmentsList";
        ...
        // Constructor
        public AppointmentsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(_StartTime.AdvancedSearch.ToJson())); // Field StartTime
            filters.Merge(JObject.Parse(EndTime.AdvancedSearch.ToJson())); // Field EndTime
            filters.Merge(JObject.Parse(CreatedBy.AdvancedSearch.ToJson())); // Field CreatedBy
            filters.Merge(JObject.Parse(Timezone.AdvancedSearch.ToJson())); // Field Timezone
        ...
        protected bool RestoreFilterList() {
            ...
            // Field StartTime
            if (filter?.TryGetValue("x__StartTime", out sv) ?? false) {
                _StartTime.AdvancedSearch.SearchValue = sv;
                _StartTime.AdvancedSearch.SearchOperator = filter["z__StartTime"];
                _StartTime.AdvancedSearch.SearchCondition = filter["v__StartTime"];
                _StartTime.AdvancedSearch.SearchValue2 = filter["y__StartTime"];
                _StartTime.AdvancedSearch.SearchOperator2 = filter["w__StartTime"];
                _StartTime.AdvancedSearch.Save();
            }

            // Field EndTime
            if (filter?.TryGetValue("x_EndTime", out sv) ?? false) {
                EndTime.AdvancedSearch.SearchValue = sv;
                EndTime.AdvancedSearch.SearchOperator = filter["z_EndTime"];
                EndTime.AdvancedSearch.SearchCondition = filter["v_EndTime"];
                EndTime.AdvancedSearch.SearchValue2 = filter["y_EndTime"];
                EndTime.AdvancedSearch.SearchOperator2 = filter["w_EndTime"];
                EndTime.AdvancedSearch.Save();
            }
            ...
            // Field Timezone
            if (filter?.TryGetValue("x_Timezone", out sv) ?? false) {
                Timezone.AdvancedSearch.SearchValue = sv;
                Timezone.AdvancedSearch.SearchOperator = filter["z_Timezone"];
                Timezone.AdvancedSearch.SearchCondition = filter["v_Timezone"];
                Timezone.AdvancedSearch.SearchValue2 = filter["y_Timezone"];
                Timezone.AdvancedSearch.SearchOperator2 = filter["w_Timezone"];
                Timezone.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Id); // Id
                UpdateSort(_Title); // Title
                UpdateSort(_StartTime); // StartTime
                UpdateSort(EndTime); // EndTime
                UpdateSort(CreatedBy); // CreatedBy
                UpdateSort(Timezone); // Timezone
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""Appointments"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""Appointments"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""Appointments"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fAppointmentslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fAppointmentslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""Appointments"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Title")); // DN
                CreateColumnOption(option.Add("StartTime")); // DN
                CreateColumnOption(option.Add("EndTime")); // DN
                CreateColumnOption(option.Add("CreatedBy")); // DN
                CreateColumnOption(option.Add("Timezone")); // DN
            }
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fAppointmentslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_Appointments");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(appointmentsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(appointmentsList.RowCount) + "_Appointments");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(appointmentsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && appointmentsList.RowType == RowType.Add || IsEdit && appointmentsList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                _StartTime.TooltipValue = "";

                // EndTime
                EndTime.HrefValue = "";
                EndTime.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fAppointmentssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
