...
public partial class project1 {
    ...
    /// refreshTokensList
    /// </summary>
    public static RefreshTokensList refreshTokensList
    {
        get => HttpData.Get<RefreshTokensList>("refreshTokensList")!;
        set => HttpData["refreshTokensList"] = value;
    }

    /// <summary>
    /// Page class for RefreshTokens
    /// </summary>
    public class RefreshTokensList : RefreshTokensListBase
    {
        // Constructor
        public RefreshTokensList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public RefreshTokensList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class RefreshTokensListBase : RefreshTokens
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "refreshTokensList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fRefreshTokenslist";
        ...
        // Constructor
        public RefreshTokensListBase()
        {
            TableName = "RefreshTokens";

            // CSS class name as context
            TableVar = "RefreshTokens";
            ...
            MultiDeleteUrl = "RefreshTokensDelete";
            MultiUpdateUrl = "RefreshTokensUpdate";
        ...
        // Page name
        public string PageName => "RefreshTokensList";
        ...
        // Constructor
        public RefreshTokensListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(_Token.AdvancedSearch.ToJson())); // Field Token
            filters.Merge(JObject.Parse(ExpiryDate.AdvancedSearch.ToJson())); // Field ExpiryDate
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Token
            if (filter?.TryGetValue("x__Token", out sv) ?? false) {
                _Token.AdvancedSearch.SearchValue = sv;
                _Token.AdvancedSearch.SearchOperator = filter["z__Token"];
                _Token.AdvancedSearch.SearchCondition = filter["v__Token"];
                _Token.AdvancedSearch.SearchValue2 = filter["y__Token"];
                _Token.AdvancedSearch.SearchOperator2 = filter["w__Token"];
                _Token.AdvancedSearch.Save();
            }

            // Field ExpiryDate
            if (filter?.TryGetValue("x_ExpiryDate", out sv) ?? false) {
                ExpiryDate.AdvancedSearch.SearchValue = sv;
                ExpiryDate.AdvancedSearch.SearchOperator = filter["z_ExpiryDate"];
                ExpiryDate.AdvancedSearch.SearchCondition = filter["v_ExpiryDate"];
                ExpiryDate.AdvancedSearch.SearchValue2 = filter["y_ExpiryDate"];
                ExpiryDate.AdvancedSearch.SearchOperator2 = filter["w_ExpiryDate"];
                ExpiryDate.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(_Token); // Token
                UpdateSort(ExpiryDate); // ExpiryDate
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""RefreshTokens"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""RefreshTokens"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""RefreshTokens"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fRefreshTokenslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fRefreshTokenslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""RefreshTokens"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Token")); // DN
                CreateColumnOption(option.Add("ExpiryDate")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fRefreshTokenssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fRefreshTokenssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fRefreshTokenslist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_RefreshTokens");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(refreshTokensList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(refreshTokensList.RowCount) + "_RefreshTokens");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(refreshTokensList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && refreshTokensList.RowType == RowType.Add || IsEdit && refreshTokensList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fRefreshTokenssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
