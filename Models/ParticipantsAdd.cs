...
public partial class UAMS_20250216_1835 {
    ...
    /// participantsAdd
    /// </summary>
    public static ParticipantsAdd participantsAdd
    {
        get => HttpData.Get<ParticipantsAdd>("participantsAdd")!;
        set => HttpData["participantsAdd"] = value;
    }

    /// <summary>
    /// Page class for Participants
    /// </summary>
    public class ParticipantsAdd : ParticipantsAddBase
    {
        // Constructor
        public ParticipantsAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ParticipantsAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class ParticipantsAddBase : Participants
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "participantsAdd";
        ...
        public ParticipantsAddBase()
        {
            ...
            // Table object (participants)
            if (participants == null || participants is Participants)
                participants = this;
        ...
        // Page name
        public string PageName => "ParticipantsAdd";
        ...
        // Constructor
        public ParticipantsAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (SameString(pageName, GetPageName(ListUrl)) ||
                            ...
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "ParticipantsView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("ParticipantsList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        string returnUrl = "";
                        returnUrl = ReturnUrl;
                        if (GetPageName(returnUrl) == "ParticipantsList")
                            returnUrl = AddMasterUrl(ListUrl); // List page, return to List page with correct master key if necessary
                        else if (GetPageName(returnUrl) == "ParticipantsView")
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "ParticipantsList") {
                                ...
                                returnUrl = "ParticipantsList"; // Return list page content
                            }
                        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'AppointmentId' before field var 'x_AppointmentId'
            val = CurrentForm.HasValue("AppointmentId") ? CurrentForm.GetValue("AppointmentId") : CurrentForm.GetValue("x_AppointmentId");
            if (!AppointmentId.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("AppointmentId") && !CurrentForm.HasValue("x_AppointmentId")) // DN
                    AppointmentId.Visible = false; // Disable update for API request
                else
                    AppointmentId.SetFormValue(val, true, validate);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            AppointmentId.CurrentValue = AppointmentId.FormValue;
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            AppointmentId.SetDbValue(row["AppointmentId"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("AppointmentId", AppointmentId.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!Empty(curVal)) {
                    if (UserId.Lookup != null && IsDictionary(UserId.Lookup?.Options) && UserId.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        UserId.EditValue = UserId.LookupCacheOption(curVal);
                    } else { // Lookup from database // DN
                        string filterWrk = SearchFilter(UserId.Lookup?.GetTable()?.Fields["Id"].SearchExpression, "=", UserId.CurrentValue, UserId.Lookup?.GetTable()?.Fields["Id"].SearchDataType, "");
                        string? sqlWrk = UserId.Lookup?.GetSql(false, filterWrk, null, this, true, true);
                        var rswrk = sqlWrk != null ? Connection.GetRows(sqlWrk) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                        if (rswrk?.Count > 0 && UserId.Lookup != null) { // Lookup values found
                            var listwrk = UserId.Lookup?.RenderViewRow(rswrk[0]);
                            UserId.EditValue = UserId.DisplayValue(listwrk);
                        } else {
                            UserId.EditValue = HtmlEncode(FormatNumber(UserId.CurrentValue, UserId.FormatPattern));
                        }
                    }
                } else {
                    UserId.EditValue = DbNullValue;
                }
                ...
                // AppointmentId
                AppointmentId.SetupEditAttributes();
                AppointmentId.EditValue = AppointmentId.CurrentValue;
                string curVal2 = ConvertToString(AppointmentId.CurrentValue);
                if (!Empty(curVal2)) {
                    if (AppointmentId.Lookup != null && IsDictionary(AppointmentId.Lookup?.Options) && AppointmentId.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        AppointmentId.EditValue = AppointmentId.LookupCacheOption(curVal2);
                    } else { // Lookup from database // DN
                        string filterWrk2 = SearchFilter(AppointmentId.Lookup?.GetTable()?.Fields["Id"].SearchExpression, "=", AppointmentId.CurrentValue, AppointmentId.Lookup?.GetTable()?.Fields["Id"].SearchDataType, "");
                        string? sqlWrk2 = AppointmentId.Lookup?.GetSql(false, filterWrk2, null, this, true, true);
                        var rswrk2 = sqlWrk2 != null ? Connection.GetRows(sqlWrk2) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                        if (rswrk2?.Count > 0 && AppointmentId.Lookup != null) { // Lookup values found
                            var listwrk = AppointmentId.Lookup?.RenderViewRow(rswrk2[0]);
                            AppointmentId.EditValue = AppointmentId.DisplayValue(listwrk);
                        } else {
                            AppointmentId.EditValue = HtmlEncode(FormatNumber(AppointmentId.CurrentValue, AppointmentId.FormatPattern));
                        }
                    }
                } else {
                    AppointmentId.EditValue = DbNullValue;
                }
                AppointmentId.PlaceHolder = RemoveHtml(AppointmentId.Caption);
                if (!Empty(AppointmentId.EditValue) && IsNumeric(AppointmentId.EditValue))
                    AppointmentId.EditValue = FormatNumber(AppointmentId.EditValue, null);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            bool validateForm = true;
                ...
                if (AppointmentId.Visible && AppointmentId.Required) {
                    if (!AppointmentId.IsDetailKey && Empty(AppointmentId.FormValue)) {
                        AppointmentId.AddErrorMessage(ConvertToString(AppointmentId.RequiredErrorMessage).Replace("%s", AppointmentId.Caption));
                    }
                }
                if (!CheckInteger(AppointmentId.FormValue)) {
                    AppointmentId.AddErrorMessage(AppointmentId.GetErrorMessage(false));
                }
        ...
        protected Dictionary<string, object> GetAddRow()
        {
            try {
                ...
                // AppointmentId
                AppointmentId.SetDbValue(rsnew, AppointmentId.CurrentValue);

                // Status
                Status.SetDbValue(rsnew, Status.CurrentValue, Empty(Status.CurrentValue));
                return rsnew;
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
        ...
        private void RestoreAddFormFromRow(Dictionary<string, object> row)
        {
            ...
            if (row.TryGetValue("UserId", out value)) // UserId
                UserId.SetFormValue(ConvertToString(value));
            if (row.TryGetValue("AppointmentId", out value)) // AppointmentId
                AppointmentId.SetFormValue(ConvertToString(value));
        ...
        protected void SetupBreadcrumb()
        {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("ParticipantsList")), "", TableVar, true);
